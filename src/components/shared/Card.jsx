  ...
  return (
      ...
      {data.map(product => (
        <div className='bg-[#1F1D2B] p-8 rounded-xl flex flex-col items-center gap-2 text-center text-gray-300' key={product.id}>
          <figure>
            <img src={product.imagen} className="w-30 h-40 object-fill -mt-20 shadow-2xl rounded-full" />
          </figure>
...
rounded-xl text-white text-center justify-center" onClick={()=>onAddProduct(product)}>Agregar</button>
